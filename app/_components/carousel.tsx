...
import { isClient, mergeDeep } from "../_lib/utils";
...
export const Carousel: FC<CarouselProps> = ({
  ...
  const instantNavigateTo = useCallback(
    (item: number) => () => {
      if (!items) return;
  ...
  useEffect(() => {
    if (!items) return;
    const validateActiveSlide = Math.max(
      0,
      Math.min(activeSlide, items.length - 1),
    );
    instantNavigateTo(validateActiveSlide)();
...
const DefaultLeftControl: FC<DefaultLeftRightControlProps> = ({
  theme: customTheme = {},
}) => {
  const theme = mergeDeep(getTheme().carousel, customTheme);
  return (
...
const DefaultRightControl: FC<DefaultLeftRightControlProps> = ({
  theme: customTheme = {},
}) => {
  const theme = mergeDeep(getTheme().carousel, customTheme);
  return (
